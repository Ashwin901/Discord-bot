...
client.on("message", async (message) => {
  ...
  if (message.content === "hello") {
    message.channel.send(
      `Hello ${message.author.tag}. Welcome to the test-server`
    );
  }
  ...
  if (message.content.startsWith(PREFIX)) {
    ...
    if (cmd_name === "kick") {
      ...
      if (args.length == 0) return message.reply("Please enter the song name");
      try{
        const voiceChannel = message.member.voice.channel;
        if(!voiceChannel) return message.reply("You have to be in a voice channel to play songs");
